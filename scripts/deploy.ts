...
async function main() {
  ...
  if (!adminAddress) {
    ...
    // Arbitrator panel — prefer explicit public keys, else generate fresh keypairs.
    const arbitratorsRaw = process.env.DISPUTE_ARBITRATORS;
    const arbitrators: string[] = [];
    const generatedSecrets: string[] = [];
    if (arbitratorsRaw) {
      for (const key of arbitratorsRaw.split(",").map((s) => s.trim()).filter(Boolean)) {
        arbitrators.push(key);
      }
      if (arbitrators.length < 3) {
        console.error("ERROR: DISPUTE_ARBITRATORS must list at least 3 public keys (G...)");
        process.exit(1);
      }
    } else {
      for (let i = 0; i < 3; i++) {
        const kp = Keypair.random();
        arbitrators.push(kp.publicKey());
        generatedSecrets.push(kp.secret());
      }
      console.warn("\n⚠️  Generated 3 fresh arbitrator keypairs for the Dispute contract.");
      console.warn("    Their private keys are NOT stored anywhere. SAVE THEM NOW:\n");
      generatedSecrets.forEach((s, i) => console.warn(`  ARBITRATOR_${i + 1}: ${s}`));
      console.warn(
        "\n    For production, pre-set DISPUTE_ARBITRATORS=G1,G2,G3 (keys you control).\n"
      );
      if (NETWORK === "mainnet") {
        console.warn(
          "    ⚠️  MAINNET: generated keys are unsafe unless you save them now. Abort and redeploy with DISPUTE_ARBITRATORS if unsure.\n"
        );
      }
    }
    const requiredVotes = process.env.DISPUTE_REQUIRED_VOTES || "2";
    const arbitratorsJson = JSON.stringify(arbitrators);
    ...
    invokeInit(
      disputeId,
      "initialize",
      `--admin ${adminAddress} --arbitrators '${arbitratorsJson}' --required_votes ${requiredVotes} --reputation_contract ${reputationId}`
    );
    ...
    // Authorize the Dispute contract in Reputation so dispute outcomes can be recorded.
    console.log("Authorizing Dispute in Reputation...");
    runWithRetry(
      `stellar contract invoke --id ${reputationId} --source ${SECRET_KEY} --network-passphrase "${NETWORK_PASSPHRASE}" --rpc-url ${RPC_URL} -- authorize_group --caller ${adminAddress} --group ${disputeId} 2>&1`
    );
  }
  ...
  // USDC issuer matching the token above. Testnet USDC (CBIELTK…) is issued by
  // GBBD47IF…; the mainnet Circle issuer is GA5ZSEJY….
  const usdcIssuer =
    NETWORK === "mainnet"
      ? "GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN"
      : "GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5";
  const anchorSep24Url = NETWORK === "mainnet" ? "" : "https://testanchor.stellar.org/sep24";
...
NEXT_PUBLIC_USDC_CONTRACT=${usdcContract}
NEXT_PUBLIC_USDC_ISSUER=${usdcIssuer}
NEXT_PUBLIC_ANCHOR_SEP24_URL=${anchorSep24Url}
...
NEXT_PUBLIC_CONTRACT_USDC=${usdcContract}
`;
